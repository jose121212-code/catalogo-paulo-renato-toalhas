...
function renderAdminGrid() {
    ...
    products.forEach(p => {
        const listaVariacoes = p.variations && p.variations.length > 0
            ? p.variations
            : [{size: 'Atacado', price: 0}];

        const atacado = listaVariacoes.find(v => (v.size || '').toUpperCase() === 'ATACADO')
            || listaVariacoes[1]
            || listaVariacoes[0];
        const idxAtacado = listaVariacoes.indexOf(atacado);

        const precosHTML = `
            <div class="price-container-grid">
                <div class="price-block" onclick="editPrice(${p.id}, ${idxAtacado})">
                    <span class="price-label">Atacado</span>
                    <span class="price-value">R$ ${parseFloat(atacado.price || 0).toFixed(2).replace('.', ',')}</span>
                </div>
            </div>
        `;
